...
def table_marker_publisher():
    ...
    if not rospy.is_shutdown():
        ...
        if cup_on_table:
            msg.markers.append(make_mesh_marker("base_link", len(msg.markers) + 1, Pose(Point(x=0.4, z=0.71), Quaternion(z=1.0)), Vector3(1, 1, 1), white(), "package://pr2_boilerplate/meshes/cup_eco_orange.dae", frame_locked=True))
        else:
            msg.markers.append(make_mesh_marker("l_gripper_tool_frame", len(msg.markers) + 1, Pose(Point(x=-0.01, z=-0.02), Quaternion(z=1.0)), Vector3(1, 1, 1), white(), "package://pr2_boilerplate/meshes/cup_eco_orange.dae", frame_locked=True))
        msg.markers.append(make_mesh_marker("r_gripper_tool_frame", len(msg.markers) + 1, Pose(Point(x=-0.01, z=0.02), Quaternion(z=1.0)), Vector3(1, 1, 1), white(), "package://pr2_boilerplate/meshes/sigg_bottle.dae", frame_locked=True))
        pub.publish(msg)
